 ...
 * VERSION 2.5 - FIX UPLOAD FOTO PROFIL (JSON PAYLOAD)
 */
...
// Simulasi Identitas Warga Aktif (Sesuaikan dengan LocalStorage atau form login-mu)
window.wargaAktif = {
...
function renderProfilWarga() {
    ...
    // Render avatar jika kolom Foto di Sheets ada isinya
    if (dataWarga.Foto && dataWarga.Foto.trim() !== "" && dataWarga.Foto !== "-") {
...
async function handleGantiFoto(input) {
    ...
    try {
        // Kompresi di client side, paksa maksimal lebar 800px untuk hemat ruang Drive
        const base64Data = await kompresiCanvasKeBase64(file, 800, 0.75);

        // Bungkus data ke dalam format JSON payload terstruktur
        const payload = {
            hp: window.wargaAktif.hp,
            filename: `FOTO_${window.wargaAktif.hp}_${Date.now()}.jpg`,
            mimetype: "image/jpeg",
            image: base64Data
        };

        // Kirim via POST dengan parameter action disuntikkan di URL query
        const respon = await fetch(`${SCRIPT_URL}?action=updateProfilWarga`, {
            method: "POST",
            headers: {
                "Content-Type": "text/plain;charset=utf-8" // text/plain ampuh mem-bypass CORS preflight di GAS
            },
        ...
        if (hasil.status === "success") {
            if (document.getElementById("avatarWarga")) {
                document.getElementById("avatarWarga").src = hasil.newFotoUrl;
            }
            Swal.fire("Berhasil", "Foto profil kamu sukses diperbarui!", "success");
            
            // Tarik ulang database agar sinkron total
            muatDatabaseTuntas();
        } else {
            throw new Error(hasil.message || "Gagal menyimpan foto di server.");
        }
    } catch (err) {
